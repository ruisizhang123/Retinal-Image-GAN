...
# only show error when training in GPU
...
def save_generated_images(generated_images, epoch, batch_number):
    ...
    plt.tight_layout()
    save_name = './OutputImages/generatedSamples_epoch' + str(
...
def train_dcgan(batch_size, epochs, image_shape, dataset_path):
    ...
    optimizer = Adam(lr=0.00015, beta_1=0.5)## adjustable
    ...
    number_of_batches = int(40 / batch_size)
    ...
    for epoch in range(epochs):
        ...
        plt.pause(0.0000000001)
        plt.show()
        plt.savefig('./OutputImages/trainingLossPlot.png')


def main():
    dataset_path = './InputImages'
    batch_size = 40
